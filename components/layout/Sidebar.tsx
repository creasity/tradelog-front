  ...
  return (
    <>
      ...
      <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
        {NAV.map(({ href, icon: Icon, label }) => {
          const active = pathname === href ||
            (href !== '/dashboard' && href !== '/trades/new' && href !== '/trades/import' && pathname.startsWith(href))
      ...
      <div className="px-3 pb-5 border-t border-light-border dark:border-dark-border pt-3 space-y-1">
        ...
        <button
          onClick={toggle}
          className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-light-hover dark:hover:bg-dark-hover transition-all w-full"
        >
          {isDark ? <Sun size={16} className="text-gray-500 dark:text-gray-400" /> : <Moon size={16} className="text-gray-500 dark:text-gray-400" />}
